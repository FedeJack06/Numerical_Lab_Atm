...
% ------------------------------
  ...
  sl_2     = sin(l_2.*deltax_2/2);
  cl_2     = cos(l_2.*deltax_2/2);
% ------------------------------
% Plotting parameters
% ------------------------------
  min_max1=[0 7.5];
...
% A-grid
       wA(jpi,jpj) = sqrt( ...
                          f^2+((4*g*H)/(deltax^2)) * ... 
                    (sk(jpi)^2.*ck(jpi)^2+sl(jpj)^2.*cl(jpj)^2) ...
                         );
% B-grid
       wB(jpi,jpj) = sqrt( ...
                          f^2+((4*g*H)/(deltax^2)) * ... 
                    (sk(jpi)^2*cl(jpj)^2+sl(jpj)^2*ck(jpi)^2) ...
                         );
% C-grid
       wC(jpi,jpj) = sqrt( ...
                          f^2*ck(jpi)^2*cl(jpj)^2 + ...
                       ((4*g*H)/deltax^2)*(sk(jpi)^2+sl(jpj)^2) ...
                         );
   end
  end
...
% A-grid
       w2A(jpi,jpj) = sqrt( ...
                    ...
                    (sk_2(jpi)^2.*ck_2(jpi)^2+sl_2(jpj)^2.*cl_2(jpj)^2) ...
                         );
% B-grid
       w2B(jpi,jpj) = sqrt( ...
                          f^2+((4*g*H)/(deltax_2^2)) * ... 
                    (sk_2(jpi)^2*cl_2(jpj)^2+sl_2(jpj)^2*ck_2(jpi)^2) ...
                         );
% C-grid
       w2C(jpi,jpj) = sqrt( ...
                          f^2*ck_2(jpi)^2*cl_2(jpj)^2 + ...
                       ((4*g*H)/deltax_2^2)*(sk_2(jpi)^2+sl_2(jpj)^2) ...
                         );
   end
  end
% ------------------------------
